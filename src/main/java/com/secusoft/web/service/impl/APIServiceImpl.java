...
//        if(param instanceof JSONArray){
	 ...
	 */
	@Override
	public JSONObject requestAPI(Object param, HttpServletRequest request) {
		if (param instanceof JSONArray) {
			JSONArray array = JSONArray.parseArray(String.valueOf(param));
			param = array;
		}
		...
		try {
			...
			// 构建消息实体
			StringEntity entity = new StringEntity(String.valueOf(param), Charset.forName("UTF-8"));
			entity.setContentEncoding("UTF-8");
			// 发送Json格式的数据请求
			entity.setContentType("application/json");
			post.setEntity(entity);
...
//			System.out.println("业务api对接返回数据：" + response + " 返回实体：" + resultStr);
			JSONObject result = JSONObject.parseObject(resultStr, JSONObject.class);
			//返回结果
			return result;
		} catch (Exception e) {
			e.printStackTrace();
